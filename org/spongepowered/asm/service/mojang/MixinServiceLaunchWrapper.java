...
IClassBytecodeProvider {
    ...
    public MixinEnvironment.Phase getInitialPhase() {
        if (MixinServiceLaunchWrapper.findInStackTrace("net.minecraft.launchwrapper.Launch", "launch") <= 132) return MixinEnvironment.Phase.PREINIT;
    ...
    public void init() {
        ...
        if ((tweakClasses = (List)GlobalProperties.get(BLACKBOARD_KEY_TWEAKCLASSES)) == null) return;
    ...
    public void checkEnv(Object bootSource) {
        if (bootSource.getClass().getClassLoader() == Launch.class.getClassLoader()) return;
    ...
    public byte[] getClassBytes(String name, String transformedName) throws IOException {
        ...
        try {
            ...
            byte[] arrby = IOUtils.toByteArray(classStream);
    ...
    private byte[] applyTransformers(String name, String transformedName, byte[] basicClass, Profiler profiler) {
        ...
        Iterator<ILegacyClassTransformer> iterator = environment.getTransformers().iterator();
        while (iterator.hasNext()) {
            ILegacyClassTransformer transformer = iterator.next();
            this.lock.clear();
    ...
    private String unmapClassName(String className) {
        ...
        if (this.nameTransformer == null) return className;
    ...
    private static int findInStackTrace(String className, String methodName) {
        ...
        StackTraceElement[] arrstackTraceElement = stackTrace = currentThread.getStackTrace();
        int n = arrstackTraceElement.length;
        int n2 = 0;
        while (n2 < n) {
            StackTraceElement s = arrstackTraceElement[n2];
