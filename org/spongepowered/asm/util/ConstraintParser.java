...
public final class ConstraintParser {
    ...
    public static Constraint parse(String expr) {
        ...
        if (expr == null) return Constraint.NONE;
        if (expr.length() == 0) {
    ...
    public static class Constraint {
        ...
        private void parse() {
            ...
            if (hasModifier) {
                ...
                if (!"<=".equals(leading)) return;
                this.max = this.min;
                this.min = Integer.MIN_VALUE;
                return;
            }
            if (!this.has(2)) return;
        ...
        private boolean has(int index) {
            if (this.constraint[index] == null) return false;
            return true;
        }
        ...
        private int val(int index) {
            if (this.constraint[index] == null) return 0;
            int n = Integer.parseInt(this.constraint[index]);
            return n;
        }
        ...
        public String getRangeHumanReadable() {
            ...
            if (this.min != this.max) return String.format("between %d and %d", this.min, this.max);
