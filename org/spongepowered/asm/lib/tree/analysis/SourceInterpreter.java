...
implements Opcodes {
    ...
    public SourceValue merge(SourceValue d, SourceValue w) {
        if (d.insns instanceof SmallSet && w.insns instanceof SmallSet) {
            ...
            if (s != d.insns) return new SourceValue(Math.min(d.size, w.size), s);
            if (d.size != w.size) return new SourceValue(Math.min(d.size, w.size), s);
            return d;
        }
        if (d.size == w.size) {
            if (d.insns.containsAll(w.insns)) return d;
        }
