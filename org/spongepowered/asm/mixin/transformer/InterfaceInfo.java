...
public final class InterfaceInfo {
    ...
    private InterfaceInfo(MixinInfo mixin, String prefix, Type iface, boolean unique) {
        if (prefix == null || prefix.length() < 2 || !prefix.endsWith("$")) throw new InvalidMixinException((IMixinInfo)mixin, String.format("Prefix %s for iface %s is not valid", prefix, iface.toString()));
        this.mixin = mixin;
        this.prefix = prefix;
        this.iface = iface;
        this.unique = unique;
    ...
    private void readInterface(String ifaceName) {
        ...
        Iterator<Object> iterator = interfaceInfo.getInterfaces().iterator();
        while (iterator.hasNext()) {
            String superIface = (String)iterator.next();
    ...
    public boolean renameMethod(MethodNode method) {
        ...
        if (!method.name.startsWith(this.prefix)) {
            if (!this.methods.contains(method.name + method.desc)) return false;
    ...
    private void decorateUniqueMethod(MethodNode method) {
        ...
        if (Annotations.getVisible(method, Unique.class) != null) return;
    ...
    static InterfaceInfo fromAnnotation(MixinInfo mixin, AnnotationNode node) {
        ...
        if (unique != null && unique.booleanValue()) {
            bl = true;
            return new InterfaceInfo(mixin, prefix, iface, bl);
        }
        bl = false;
        return new InterfaceInfo(mixin, prefix, iface, bl);
    }
    ...
    public boolean equals(Object o) {
        ...
        if (!this.mixin.equals(that.mixin)) return false;
        if (!this.prefix.equals(that.prefix)) return false;
        if (!this.iface.equals(that.iface)) return false;
        return true;
    }
    ...
    public int hashCode() {
        ...
        return 31 * result + this.iface.hashCode();
    }
}
