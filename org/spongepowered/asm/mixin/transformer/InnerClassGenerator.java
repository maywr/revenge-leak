...
implements IClassGenerator {
    ...
    private static String getUniqueReference(String originalName, MixinTargetContext context) {
        ...
        if (!name.matches("^[0-9]+$")) return String.format("%s$%s$%s", context.getTargetClassRef(), name, UUID.randomUUID().toString().replace("-", ""));
        name = "Anonymous";
    ...
    extends Remapper {
        ...
        public String mapMethodName(String owner, String name, String desc) {
            if (!this.ownerName.equalsIgnoreCase(owner)) return super.mapMethodName(owner, name, desc);
            ClassInfo.Method method = this.owner.getClassInfo().findMethod(name, desc, 10);
            if (method == null) return super.mapMethodName(owner, name, desc);
        ...
        public String map(String key) {
            ...
            if (!this.ownerName.equals(key)) return key;
            return this.targetName;
        }
