...
class MixinPreProcessorStandard {
    ...
    final MixinPreProcessorStandard prepare() {
        ...
        Iterator<MixinInfo.MixinMethodNode> iterator = this.classNode.fields.iterator();
        do {
            if (!iterator.hasNext()) {
                prepareTimer.end();
                return this;
            }
            FieldNode mixinField = (FieldNode)((Object)iterator.next());
    ...
    protected void prepareShadow(MixinInfo.MixinMethodNode mixinMethod, ClassInfo.Method method) {
        ...
        if (!mixinMethod.name.startsWith(prefix)) return;
    ...
    protected void prepareSoftImplements(MixinInfo.MixinMethodNode mixinMethod, ClassInfo.Method method) {
        ...
        while (iterator.hasNext()) {
            InterfaceInfo iface = iterator.next();
    ...
    final MixinPreProcessorStandard conform(ClassInfo target) {
        ...
        do {
            ...
            MixinInfo.MixinMethodNode mixinMethod = iterator.next();
    ...
    protected boolean attachAccessorMethod(MixinTargetContext context, MixinInfo.MixinMethodNode mixinMethod) {
        if (this.attachAccessorMethod(context, mixinMethod, SpecialMethod.ACCESSOR)) return true;
        if (this.attachAccessorMethod(context, mixinMethod, SpecialMethod.INVOKER)) return true;
        return false;
    }
    ...
    protected boolean attachSpecialMethod(MixinTargetContext context, MixinInfo.MixinMethodNode mixinMethod, SpecialMethod type) {
        ...
        if (target.name.equals(mixinMethod.name)) return true;
    ...
    private void conformVisibility(MixinTargetContext context, MixinInfo.MixinMethodNode mixinMethod, SpecialMethod type, MethodNode target) {
        ...
        if (visMethod.ordinal() >= visTarget.ordinal()) {
            if (visTarget != Bytecode.Visibility.PRIVATE) return;
            if (visMethod.ordinal() <= Bytecode.Visibility.PRIVATE.ordinal()) return;
        ...
        if (visMethod != Bytecode.Visibility.PRIVATE) return;
        if (type.isOverwrite) {
    ...
    protected boolean attachUniqueMethod(MixinTargetContext context, MixinInfo.MixinMethodNode mixinMethod) {
        ...
        try {
            ...
            if (!this.verboseLogging) {
                if (!this.env.getOption(MixinEnvironment.Option.DEBUG_VERIFY)) throw new InvalidMixinException((IMixinInfo)this.mixin, ex.getMessage());
            }
    ...
    protected void attachMethod(MixinTargetContext context, MixinInfo.MixinMethodNode mixinMethod) {
        ...
        if ((target = context.findMethod(mixinMethod, null)) == null) return;
    ...
    protected boolean validateField(MixinTargetContext context, FieldNode field, AnnotationNode shadow) {
        ...
        if (!"super$".equals(field.name)) return true;
        if (field.access != 2) {
        ...
        if (field.desc.equals("L" + this.mixin.getClassRef() + ";")) return false;
    ...
    private static String getDynamicInfo(String targetType, AnnotationNode annotation) {
        ...
        String string = String.format(" %s is @Dynamic(%s)", targetType, description);
        return string;
    }
