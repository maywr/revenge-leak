...
extends Injector {
    ...
    protected void sanityCheck(Target target, List<InjectionPoint> injectionPoints) {
        ...
        if (!"<init>".equals(target.method.name)) return;
        Iterator<InjectionPoint> iterator = injectionPoints.iterator();
        do {
            if (!iterator.hasNext()) return;
        } while ((injectionPoint = iterator.next()).getClass().equals(BeforeReturn.class));
...
lbl25: // 1 sources:
                ...
                returnableSig = this.methodNode.desc.replace("Lorg/spongepowered/asm/mixin/injection/callback/CallbackInfo;", "Lorg/spongepowered/asm/mixin/injection/callback/CallbackInfoReturnable;");
                ...
                surrogateHandler = Bytecode.findMethod(this.classNode, this.methodNode.name, callback.getDescriptor());
                if (surrogateHandler == null) throw new InvalidInjectionException(this.info, "Invalid descriptor on " + this.info + "! Expected " + callback.getDescriptor() + " but found " + this.methodNode.desc);
                if (Annotations.getVisible(surrogateHandler, Surrogate.class) == null) throw new InvalidInjectionException(this.info, "Invalid descriptor on " + this.info + "! Expected " + callback.getDescriptor() + " but found " + this.methodNode.desc);
                callbackMethod = surrogateHandler;
            }
        }
    ...
    extends InsnList {
        ...
        int marshalVar() {
            if (this.marshalVar >= 0) return this.marshalVar;
