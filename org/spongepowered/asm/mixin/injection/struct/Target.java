...
Iterable<AbstractInsnNode> {
    ...
    public void setMaxLocals(int maxLocals) {
        if (maxLocals <= this.method.maxLocals) return;
    ...
    public void setMaxStack(int maxStack) {
        if (maxStack <= this.method.maxStack) return;
    ...
    private int allocateArgMapLocal(int index, int size) {
        ...
        if (index + size <= this.argMapVars.size()) return local;
    ...
    public int[] getArgIndices() {
        if (this.argIndices != null) return this.argIndices;
    ...
    private int[] calcArgIndices(int local) {
        ...
        int arg = 0;
        while (arg < this.arguments.length) {
    ...
    public String getCallbackInfoClass() {
        if (this.callbackInfoClass != null) return this.callbackInfoClass;
    ...
    public String getCallbackDescriptor(boolean captureLocals, Type[] locals, Type[] argumentTypes, int startIndex, int extra) {
        ...
        while (l < locals.length) {
            if (extra <= 0) return descriptor.append(")V").toString();
            if (locals[l] != null) {
    ...
    public int compareTo(Target o) {
        if (o != null) return this.toString().compareTo(o.toString());
    ...
    public MethodInsnNode findInitNodeFor(TypeInsnNode newNode) {
        start = this.indexOf(newNode);
        iter = this.insns.iterator(start);
        ...
        } while (!"<init>".equals(methodNode.name) || !methodNode.owner.equals(newNode.desc));
        return methodNode;
    }

    public MethodInsnNode findSuperInitNode() {
        if (this.isCtor) return Bytecode.findSuperInit(this.method, ClassInfo.forName(this.classNode.name).getSuperName());
        return null;
    }
