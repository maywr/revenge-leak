...
extends InvokeInjector {
    ...
    private ConstructorRedirectData getCtorRedirect(BeforeNew ip) {
        ...
        if (ctorRedirect != null) return ctorRedirect;
    ...
    protected void inject(Target target, InjectionNodes.InjectionNode node) {
        ...
        if (!(node.getCurrentTarget() instanceof TypeInsnNode)) throw new InvalidInjectionException(this.info, this.annotationType + " annotation on is targetting an invalid insn in " + target + " in " + this);
        if (node.getCurrentTarget().getOpcode() != 187) throw new InvalidInjectionException(this.info, this.annotationType + " annotation on is targetting an invalid insn in " + target + " in " + this);
    ...
    protected boolean preInject(InjectionNodes.InjectionNode node) {
        ...
        if (other.getOwner() == this) return true;
    ...
    protected void postInject(Target target, InjectionNodes.InjectionNode node) {
        ...
        if (!(node.getOriginalTarget() instanceof TypeInsnNode)) return;
        if (node.getOriginalTarget().getOpcode() != 187) return;
    ...
    protected void injectAtInvoke(Target target, InjectionNodes.InjectionNode node) {
        ...
        if (!desc.equals(this.methodNode.desc)) {
            ...
            if (!alternateDesc.equals(this.methodNode.desc)) throw new InvalidInjectionException(this.info, this.annotationType + " handler method " + this + " has an invalid signature, expected " + desc + " found " + this.methodNode.desc);
    ...
    public void injectAtScalarField(Target target, FieldInsnNode fieldNode, int opCode, Type ownerType, Type fieldType) {
        ...
        if (opCode == 178 || opCode == 180) {
            ...
            if (opCode != 179) {
                if (opCode != 181) throw new InvalidInjectionException(this.info, "Unspported opcode " + Bytecode.getOpcodeName(opCode) + " for " + this.info);
            }
    ...
    protected boolean checkDescriptor(String desc, Target target, String type) {
        ...
        String alternateDesc = String.format("%s%s%s", desc.substring(0, pos), Joiner.on("").join((Object[])target.arguments), desc.substring(pos));
        if (!this.methodNode.desc.equals(alternateDesc)) throw new InvalidInjectionException(this.info, this.annotationType + " method " + type + " " + this + " has an invalid signature. Expected " + desc + " but found " + this.methodNode.desc);
        return true;
    }
    ...
    private static String getGetArrayHandlerDescriptor(AbstractInsnNode varNode, Type returnType, Type fieldType) {
        if (varNode == null) return Bytecode.generateDescriptor(returnType, RedirectInjector.getArrayArgs(fieldType, 1, new Type[0]));
        if (varNode.getOpcode() != 190) return Bytecode.generateDescriptor(returnType, RedirectInjector.getArrayArgs(fieldType, 1, new Type[0]));
