...
extends RedirectInjector {
    ...
    protected void inject(Target target, InjectionNodes.InjectionNode node) {
        ...
        if (!Bytecode.isConstant(targetNode)) throw new InvalidInjectionException(this.info, this.annotationType + " annotation is targetting an invalid insn in " + target + " in " + this);
        this.checkTargetModifiers(target, false);
    ...
    private void injectExpandedConstantModifier(Target target, JumpInsnNode jumpNode) {
        ...
        if (opcode < 155) throw new InvalidInjectionException(this.info, this.annotationType + " annotation selected an invalid opcode " + Bytecode.getOpcodeName(opcode) + " in " + target + " in " + this);
        if (opcode > 158) {
    ...
    private AbstractInsnNode invokeConstantHandler(Type constantType, Target target, InsnList before, InsnList after) {
        ...
        if (!withArgs) return this.invokeHandler(after);
