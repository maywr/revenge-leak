...
extends InvokeInjector {
    ...
    protected void sanityCheck(Target target, List<InjectionPoint> injectionPoints) {
        ...
        if (!this.singleArgMode) return;
        if (this.methodArgs[0].equals(this.returnType)) return;
    ...
    protected int findArgIndex(Target target, Type[] args) {
        if (this.index > -1) {
            if (this.index >= args.length) throw new InvalidInjectionException(this.info, "Specified index " + this.index + " for @ModifyArg is invalid for args " + Bytecode.getDescriptor(args) + ", expected " + this.returnType + " on " + this);
            if (args[this.index].equals(this.returnType)) return this.index;
