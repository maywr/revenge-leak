...
extends Injector {
    ...
    protected boolean findTargetNodes(MethodNode into, InjectionPoint injectionPoint, InsnList insns, Collection<AbstractInsnNode> nodes) {
        if (!(injectionPoint instanceof ContextualInjectionPoint)) return injectionPoint.find(into.desc, insns, nodes);
    ...
    protected void sanityCheck(Target target, List<InjectionPoint> injectionPoints) {
        ...
        if (this.discriminator.getIndex() != 0) return;
    ...
    protected void inject(Target target, InjectionNodes.InjectionNode node) {
        ...
        try {
            ...
            if (!this.discriminator.printLVT()) throw new InvalidInjectionException(this.info, "Implicit variable modifier injection failed in " + this, (Throwable)ex);
