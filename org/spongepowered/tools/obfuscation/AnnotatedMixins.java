...
IJavadocProvider {
    ...
    protected TargetMap initTargetMap() {
        ...
        if (targetsFileName == null) return targets;
        try {
    ...
    private void initTokenCache(String tokens) {
        ...
        String[] arrstring = tokenValues = tokens.replaceAll("\\s", "").toUpperCase().split("[;,]");
        int n = arrstring.length;
        int n2 = 0;
        while (n2 < n) {
            String tokenValue = arrstring[n2];
    ...
    public String getOption(String option) {
        ...
        if (value == null) return this.getProperties().getProperty(option);
        return value;
    }
    ...
    public Properties getProperties() {
        ...
        try {
            ...
            if (propertyFile == null) return this.properties;
            InputStream inputStream = propertyFile.openInputStream();
            this.properties.load(inputStream);
            inputStream.close();
            return this.properties;
        }
        catch (Exception filer) {
            // empty catch block
        }
        return this.properties;
    }
    ...
    private IMixinAnnotationProcessor.CompilerEnvironment detectEnvironment(ProcessingEnvironment processingEnv) {
        if (!processingEnv.getClass().getName().contains("jdt")) return IMixinAnnotationProcessor.CompilerEnvironment.JAVAC;
    ...
    public void registerMixin(TypeElement mixinType) {
        ...
        if (this.mixins.containsKey(name)) return;
    ...
    public Collection<TypeMirror> getMixinsTargeting(TypeElement targetType) {
        ...
        Iterator<TypeReference> iterator = this.targets.getMixinsTargeting(targetType).iterator();
        while (iterator.hasNext()) {
            TypeReference mixin = iterator.next();
    ...
    public void onPassCompleted(RoundEnvironment roundEnv) {
        ...
        Iterator<AnnotatedMixin> iterator = (roundEnv.processingOver() ? this.mixins.values() : this.mixinsForPass).iterator();
        while (iterator.hasNext()) {
            AnnotatedMixin mixin = iterator.next();
    ...
    public void printMessage(Diagnostic.Kind kind, CharSequence msg) {
        if (this.env != IMixinAnnotationProcessor.CompilerEnvironment.JAVAC) {
            if (kind == Diagnostic.Kind.NOTE) return;
        }
    ...
    public TypeHandle getTypeHandle(String name) {
        ...
        if ((lastDotPos = name.lastIndexOf(46)) <= -1) return null;
    ...
    public TypeHandle getSimulatedHandle(String name, TypeMirror simulatedTarget) {
        ...
        if (lastDotPos <= -1) return new TypeHandleSimulated(name, simulatedTarget);
        String pkg = name.substring(0, lastDotPos);
        PackageElement packageElement = this.processingEnv.getElementUtils().getPackageElement(pkg);
        if (packageElement == null) return new TypeHandleSimulated(name, simulatedTarget);
