...
implements IMappingProvider {
    ...
    public boolean isEmpty() {
        if (!this.packageMap.isEmpty()) return false;
        if (!this.classMap.isEmpty()) return false;
        if (!this.fieldMap.isEmpty()) return false;
        if (!this.methodMap.isEmpty()) return false;
        return true;
    }
