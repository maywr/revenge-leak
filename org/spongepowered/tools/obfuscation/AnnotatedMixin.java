...
class AnnotatedMixin {
    ...
    AnnotatedMixin runValidators(IMixinValidator.ValidationPass pass, Collection<IMixinValidator> validators) {
        ...
        if (pass != IMixinValidator.ValidationPass.FINAL) return this;
    ...
    public boolean isInterface() {
        if (this.mixin.getKind() != ElementKind.INTERFACE) return false;
        return true;
    }
    ...
    public boolean isMultiTarget() {
        if (this.targets.size() <= 1) return false;
        return true;
    }
    ...
    public void registerInjector(ExecutableElement method, AnnotationHandle inject, InjectorRemap remap) {
        ...
        Iterator<AnnotationHandle> at2 = slices.iterator();
        while (at2.hasNext()) {
            AnnotationHandle to;
            AnnotationHandle slice = at2.next();
